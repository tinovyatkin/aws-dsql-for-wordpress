...
if(!isset($argv[1])) {
 $root=dirname(__DIR__,2);$library=$root.'/native/target/release/libwp_dsql_native.'.(PHP_OS_FAMILY==='Darwin'?'dylib':'so');
 foreach(['duplicate','transaction','additive'] as $case){$p=proc_open([PHP_BINARY,'-d','extension='.$library,__FILE__,$case],[0=>STDIN,1=>STDOUT,2=>STDERR],$pipes);if(proc_close($p)!==0)exit(1);}exit;
}
$case=$argv[1];
...
try {
 ...
 expect_rejection(fn()=>$d->query("UPDATE $t SET label='version-advanced' WHERE id=1"),'earlier schema');
 echo "PASS reconnect cannot bypass the request-wide failed-write guard\n";
 }
 if($case==='transaction') {
 ...
 $d->query("ALTER TABLE $t RENAME COLUMN id TO renamed_id, ADD renamed_fill int NOT NULL DEFAULT 7");
 if($d->query("SELECT COUNT(*) FROM $t WHERE renamed_fill=7")->fetchColumn()!=='3')throw new RuntimeException('Renamed primary-key backfill failed');
 echo "PASS primary-key rename followed by required-column backfill\n";
 }
