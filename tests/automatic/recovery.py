...
for point in ['reserved','planned','column-added','backfill-batch','validation-submitted','after-step','catalog-published','journal-completed']:
 call('setup');call('fault',point,expected=86);call('verify',*(['original'] if point=='reserved' else []));call('cleanup');checks.append(point);print('PASS interrupted '+point,flush=True)
